...
const { notifyAdminReturn } = require("../controllers/bookController");
...
router.get("/issued-books", auth, async (req, res) => {
  const userId = req.user.id;
  console.log('Came to issued books route');
  try {
    ...
    console.log('Raw issuedBooks:', JSON.stringify(user.issuedBooks));
    
    // Check if issuedBooks exists and is an array
    if (!user.issuedBooks || !Array.isArray(user.issuedBooks)) {
      console.log('Invalid issuedBooks structure:', user.issuedBooks);
      return res.status(500).json({ msg: "Invalid issuedBooks data structure" });
    }
  
    const issuedBooks = user.issuedBooks.map((book) => {
      console.log('Processing book:', book?._id || 'unknown');
      return {
        _id: book._id,
